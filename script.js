...
const qingtanGame = {
  title: "轻坦快跑",
  desc: "之前做的横屏跑酷小游戏，轻坦跳跃、下蹲、主防，躲避障碍并挑战最高分。",
  cover: "",
  tags: ["Game", "跑酷", "轻坦"],
  url: "games/qingtankuaipao/"
};

const puzzleGame = {
  title: "照片拼图小游戏",
  desc: "上传一张图片，自定义横向和纵向切块数量，点击或拖动碎片完成拼图。",
  cover: "",
  tags: ["Puzzle", "照片", "小游戏"],
...
function ensureGame(config, game) {
  const exists = config.games.some(item => item?.url === game.url || item?.title === game.title);
  if (!exists) config.games = [game, ...config.games];
}
...
function renderPhotos() {
  ...
  grid.innerHTML = photos.map(item => {
    const imageMarkup = item.image
      ? `<button class="photo-open" type="button" data-image="${escapeAttribute(item.image)}" data-title="${escapeAttribute(item.title)}" data-meta="${escapeAttribute(item.meta || "")}" aria-label="查看大图：${escapeAttribute(item.title)}"><img class="photo-image" src="${escapeAttribute(item.image)}" alt="${escapeAttribute(item.title)}" loading="lazy"></button>`
    ...
    const removeButton = isAdminPage && item.id
      ...
      : "";
    return `<article class="photo-card ${item.image ? "has-image" : "image-placeholder"}">${imageMarkup}${removeButton}<div class="photo-title">${escapeHtml(item.title)}<span class="photo-meta">${escapeHtml(item.meta || "")}</span></div></article>`;
  }).join("");
...
function renderTools() {
  ...
  if (!list) return;
  list.innerHTML = siteConfig.tools.map((item, index) => `<article class="tool-item"><div class="tool-index">${String(index + 1).padStart(2, "0")}</div><div><h3>${escapeHtml(item.name)}</h3><p>${escapeHtml(item.desc)}</p></div>${renderOpenLink(item.url)}</article>`).join("");
}
